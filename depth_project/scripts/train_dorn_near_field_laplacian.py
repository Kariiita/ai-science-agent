'''Train DORN with near-field (0-5m) Laplacian pyramid loss.

Usage:
    python scripts/train_dorn_near_field_laplacian.py --data_dir data --epochs 50 --batch_size 4
'''
import argparse
import importlib
import os
...
from tools.laplacian_depth_loss import near_field_laplacian_loss
...
def main():
    parser = argparse.ArgumentParser(description="Train DORN with near-field Laplacian loss on NYU Depth v2")
    ...
    # Use near-field Laplacian loss instead of L1
    criterion = near_field_laplacian_loss
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % eval_every == 0 or (epoch + 1) == args.epochs:
            ...
            if m["RMSE"] < best_rmse:
                ...
                log(f"Saved best_model.pth (val_RMSE={best_rmse:.4f})")
