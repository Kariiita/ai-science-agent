'''Train DORN baseline on NYU Depth v2 with GradNorm loss and depth-aware gradient clipping.

Usage:
    python scripts/train_dorn_gradnorm_depthclip.py --data_dir data --epochs 50 --batch_size 4
'''
import argparse
import importlib
import os
...
from tools.gradnorm_loss import gradnorm_loss, get_decoder_grad_norms
...
def main():
    ...
    # Identify last 3 decoder layers for depth-aware clipping
    decoder_layers = [
        'upconv2.weight',
        'upconv3.weight',
        'upconv4.weight',
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, depths) in enumerate(train_loader):
            ...
            # Depth-aware gradient clipping: only clip last 3 decoder layers
            # Get gradients for decoder layers
            decoder_grad_norms = {}
            for name, param in model.named_parameters():
                if name in decoder_layers:
                    if param.grad is not None:
                        decoder_grad_norms[name] = param.grad.norm().item()
        ...
        if (epoch + 1) % eval_every == 0 or (epoch + 1) == args.epochs:
            ...
            if m["AbsRel"] < best_abs_rel:
                ...
                torch.save(model.state_dict(),
                           os.path.join("model_snapshots", "best_model_gradnorm_depthclip.pth"))
                log(f"Saved best_model_gradnorm_depthclip.pth (val_AbsRel={best_abs_rel:.4f})")
