'''Train DORN baseline on NYU Depth v2 with GradNorm loss, depth-aware gradient clipping, and near-field depth-weighted L1 loss.

Usage:
    python scripts/train_dorn_gradnorm_depthclip_nearfield.py --data_dir data --epochs 50 --batch_size 4
'''
import argparse
import importlib
import os
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, depths) in enumerate(train_loader):
            ...
            # Compute L1 reconstruction loss (unchanged)
            l1_loss = l1_criterion(outputs, depths)
            
            # Compute GradNorm gradient consistency loss with near-field masking (< 3.0m)
        ...
        if (epoch + 1) % eval_every == 0 or (epoch + 1) == args.epochs:
            ...
            if m["AbsRel"] < best_abs_rel:
                ...
                torch.save(model.state_dict(),
                           os.path.join("model_snapshots", "best_model_gradnorm_depthclip_nearfield.pth"))
                log(f"Saved best_model_gradnorm_depthclip_nearfield.pth (val_AbsRel={best_abs_rel:.4f})")
